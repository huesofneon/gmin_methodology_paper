...
gmin_modelled_summ <- readRDS(paste0(data_dir,"gmin_modelled_summ_run4_agw.rds"))
gmin_modelled_full <- readRDS(paste0(data_dir,"gmin_modelled_full_run4_agw.rds"))
...
#save
ggsave("Fig6_gmin_comp_run4_agw_SA.png", device = "png", 
       path = fig_dir, 
...
#############
#############
############# make figure with projected. 


#aov & Tukey HSD for Calcd.PLA----
{
  
  #1-way ANOVA (gmin)
  gmin_aov <- aov(gmin_CalcdPLA~SPP, data = aov_data)
  summary(gmin_aov)
...
ggplot(filter(plot_letters_df, SPP != "ELYRHI"), 
  ...
  geom_errorbar(aes(ymin = CalcdPLA_RWC80_50_gmin - CalcdPLA_RWC80_50_gmin.se, 
                    ymax = CalcdPLA_RWC80_50_gmin + CalcdPLA_RWC80_50_gmin.se), 
                width = 0.2) +
  labs(x = element_blank(),
       y = bquote(g[min]~"(mmol m"^{-2}*" s"^{-1}*")"),
       title = " ") +
  coord_cartesian(ylim = c(0, 75)) +
  geom_text(aes(x = SPP, 
                y = CalcdPLA_RWC80_50_gmin + CalcdPLA_RWC80_50_gmin.se + 1.5, 
                label = Letters
  ), 
  vjust = 0) +
  theme_bw() +
...
#save
ggsave("Fig6_gmin_comp_run4_agw_PA.png", device = "png", 
       path = fig_dir, 
       width = 30, height = 15, units = "cm", dpi = 300)
