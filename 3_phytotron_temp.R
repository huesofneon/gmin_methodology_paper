...
files <- list.files(path=raw_data_dir,pattern="^3_.*\\.txt$")#find files for run 3
files
...
# create treatment label and datestamp and VPD
  data_2 <- data %>%
    ...
    mutate(Datetime = dmy_hms(Time)) %>% #check date, month, year, etc. function matches input file
...
#save data_summ----
  saveRDS(data_summ,paste0(data_dir,"3_metdata_summary.rds"))
...
ggplot(data_comb, aes(x=Datetime, y=Temp,col=Treat))+
  geom_point()+
  geom_line()
#ggsave("temps_run3.jpeg", device = "jpeg", path = paste0(fig_dir,"metdata/"), width = 20, height = 20, units = "cm", dpi = 300)
...
ggplot(data_comb, aes(x=Datetime, y=Humid,col=Treat))+
  geom_point()+
  geom_line()
#ggsave("humidity_run3.jpeg", device = "jpeg", path = paste0(fig_dir,"metdata/"), width = 20, height = 20, units = "cm", dpi = 300)

#view VPD data
ggplot(data_comb, aes(x=Datetime, y=VPD,col=Treat))+
  geom_point()+
  geom_line()
#ggsave("VPD_run3.jpeg", device = "jpeg", path = paste0(fig_dir,"metdata/"), width = 20, height = 20, units = "cm", dpi = 300)
