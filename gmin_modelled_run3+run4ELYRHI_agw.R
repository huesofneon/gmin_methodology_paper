...
df_comb <- readRDS(paste0(data_dir,"df_comb_run3+run4ELYRHI_agw.rds"))
...
p50_summ <- p50_bin_values %>%
  ...
  summarise(p50_gmin = mean(gmin_CalcdPLA),
            p50_gmin.se = se(gmin_CalcdPLA), 
            p50_gmin_SA = mean(gmin_CorrPLA),
            p50_gmin_SA.se = se(gmin_CorrPLA))
}
...
#summarise p88_gmin
  ...
  mutate(Treatment = paste0(SPP," - ",Temp))%>%
  group_by(SPP, Temp, Treatment) %>%
  summarise(p88_gmin = mean(gmin_CalcdPLA),
            p88_gmin.se = se(gmin_CalcdPLA),
            p88_gmin_SA = mean(gmin_CorrPLA),
            p88_gmin_SA.se = se(gmin_CorrPLA))
}  
...
#summarise RWC80_50_gmin
  ...
  mutate(Treatment = paste0(SPP," - ",Temp))%>%
  group_by(SPP, Temp, Treatment) %>%
  summarise(RWC80_50_gmin = mean(gmin_CalcdPLA),
            RWC80_50_gmin.se = se(gmin_CalcdPLA),
            RWC80_50_gmin_SA = mean(gmin_CorrPLA),
            RWC80_50_gmin_SA.se = se(gmin_CorrPLA))
}  
...
saveRDS(gmin_modelled_summ, paste0(data_dir,"gmin_modelled_summ_run3+run4ELYRHI_agw.rds"))
...
saveRDS(gmin_modelled_full, paste0(data_dir,"gmin_modelled_full_run3+run4ELYRHI_agw.rds"))
